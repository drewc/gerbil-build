...
*** ~gxc-compile~
    ...
    let's start to break things up into parts.

    First, a ~base~ where all things spring from.

#+begin_src gerbil :tangle "./make/base.ss" :mkdirp yes
  package: std/make
...
#+begin_src gerbil :tangle "make/mod.ss" :noweb yes
  package: drewc/make
  (import :drewc/make/expander-module ./settings :std/misc/func :std/misc/path)
  (export #t)
...
#+begin_src gerbil :tangle ./test/test-first-build.ss :noweb yes
  <<unite-settings-struct>>
...
#+end_src
 ...
 #+begin_src shell :results output code
   ...
   gxi -e '(load "~/src/gerbil-build/test/make1.ss")' \
       -e '(load "~/src/gerbil-build/test/test-make-gxc.ss")' \
       -e '(load "~/src/gerbil-build/test/import.ss")' \
       -e '(load "~/src/gerbil-build/test/test-first-build.ss")
   '
 #+end_src
 ...
 ,*** ERROR IN "make/mod.ss"@2.9
 --- Syntax Error: Cannot find library module
 ... form:   :drewc/make/expander-module
 > 
 ,*** EOF again to exit
 #+end_src
